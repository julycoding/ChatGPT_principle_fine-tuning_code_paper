...
$$\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,\,   Today\\Yesterday\,\,\begin{array}{c}\\sun\\cloud\\rain\\\end{array}\left[ \begin{matrix}sun&		cloud&		rain&		\\0.50&		0.375&		0.125&		\\0.25&		0.125&		0.625&		\\0.25&		0.375&		0.375&		\\\end{matrix} \right]$$
...
然而策略梯度有个问题，在于$\mathbb{E}_{\tau \sim p_{\theta}(\tau)}$是对策略${\pi _{\theta}}$采样的轨迹$tau$求期望。一旦更新了参数，从$\theta$变成$\theta'$，在对应状态s下采取动作的概率$p_\theta(\tau)$就不对了，之前采样的数据也不能用了。
...
>
> $\begin{aligned} \operatorname{Var}_{x \sim q}\left[f(x) \frac{p(x)}{q(x)}\right] &=\mathbb{E}_{x \sim q}\left[\left(f(x) \frac{p(x)}{q(x)}\right)^{2}\right]-\left(\mathbb{E}_{x \sim q}\left[f(x) \frac{p(x)}{q(x)}\right]\right)^{2} \\ &= \int \left(f(x) \frac{p(x)}{q(x)}\right)^{2} q(x) dx - \left(\mathbb{E}_{x \sim p}[f(x)]\right)^{2} \\ &= \int f(x)^{2} \frac{p(x)}{q(x)} p(x)dx - \left(\mathbb{E}_{x \sim p}[f(x)]\right)^{2} \\ &=\mathbb{E}_{x \sim p}\left[f(x)^{2} \frac{p(x)}{q(x)}\right]-\left(\mathbb{E}_{x \sim p}[f(x)]\right)^{2} \end{aligned}$/eq?%5Cbegin%7Baligned%7D%20%5Coperatorname%7BVar%7D_%7Bx%20%5Csim%20q%7D%5Cleft%5Bf%28x%29%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%5Cright%5D%20%26%3D%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20q%7D%5Cleft%5B%5Cleft%28f%28x%29%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%5Cright%29%5E%7B2%7D%5Cright%5D-%5Cleft%28%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20q%7D%5Cleft%5Bf%28x%29%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%5Cright%5D%5Cright%29%5E%7B2%7D%20%5C%5C%20%26%3D%20%5Cint%20%5Cleft%28f%28x%29%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%5Cright%29%5E%7B2%7D%20q%28x%29%20dx%20-%20%5Cleft%28%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20p%7D%5Bf%28x%29%5D%5Cright%29%5E%7B2%7D%20%5C%5C%20%26%3D%20%5Cint%20f%28x%29%5E%7B2%7D%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%20p%28x%29dx%20-%20%5Cleft%28%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20p%7D%5Bf%28x%29%5D%5Cright%29%5E%7B2%7D%20%5C%5C%20%26%3D%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20p%7D%5Cleft%5Bf%28x%29%5E%7B2%7D%20%5Cfrac%7Bp%28x%29%7D%7Bq%28x%29%7D%5Cright%5D-%5Cleft%28%5Cmathbb%7BE%7D_%7Bx%20%5Csim%20p%7D%5Bf%28x%29%5D%5Cright%29%5E%7B2%7D%20%5Cend%7Baligned%7D)
>
> 上述两个公式前后对比，可以很明显的看出
