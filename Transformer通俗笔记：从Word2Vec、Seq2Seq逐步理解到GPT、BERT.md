...
3.  而如果手头任务的训练集数据量较少的话，那现阶段的好用的CNN比如Resnet/Densenet/Inception等网络结构层数很深(几百万上千万甚至上亿的参数量)，当训练数据少则很难很好地训练这么复杂的网络，但如果  
    $`\rightarrow`$  先把好用的这些大模型的大量参数通过大的训练集合比如『ImageNet』**预训练**好(大规模图像数据集ImageNet有超过1000万的图像和1000类物体的标注)，即直接用ImageNet初始化大模型的大部分参数  
    $`\rightarrow`$  接下来再通过手头上少的可怜的数据去Fine-tuning(即**微调**参数)，以更适合解决当前的任务，那事情就顺理成章了
...
![](assets/images/transformer/3a9d8e9d4940ac8e5eb9985fb2257837_MD5.png)

假设一个叫Jay的人，其内向/外向得分为38/100，则可以用下图表示这个得分：

![](assets/images/transformer/2b2ea7b8a48be07468c7729fcea4f602_MD5.png)

为了更好的表达数据，我们把范围收缩到-1到1：

![](assets/images/transformer/9cc6a1fec7f70374f0aaffceace27a3b_MD5.png)

考虑到人性复杂，对于一个人的描述只有一条信息显然是不够的，为此，我们添加另一测试的得分作为一个新的第二维度，而这两个维度均可以表现为图上的一个点（或称为从原点到该点的向量)

![](assets/images/transformer/95d0fee620aaa7ae29d675c7f9827cfe_MD5.png)
...
![](assets/images/transformer/d31e0e522cb77b9aafe3cb6196376373_MD5.png)

而**计算两个向量之间相似度得分的常用方法是余弦相似度**，可曾还记得夹角余弦的计算公式？

![](assets/images/transformer/98dab2254c03b650a248195525a3ec89_MD5.png)

通过该计算公式可得

![](assets/images/transformer/21772349724bbb0e999a384fc8b92f83_MD5.png)
...
![](assets/images/transformer/00219aefb2e4074942c05a232def806e_MD5.png)
...
![](assets/images/transformer/508fe2f946b84eaa41a0ff6607c02304_MD5.png)
...
![](assets/images/transformer/664d9bbbe5a7bfe9c1871d57a21434bc_MD5.png)
...
![](assets/images/transformer/5e7d7ab1b09c5ce83f03c84a15d1ead8_MD5.jpg)
...
![](assets/images/transformer/8f75803b17a4bb9147582fec634a70a1_MD5.png)
...
![](assets/images/transformer/756c7616dca078c5a826d5d89e55c5a2_MD5.png)

让我们根据它们的值对单元格进行颜色编码（如果它们接近2则为红色，接近0则为白色，接近-2则为蓝色）：

![](assets/images/transformer/7d8d81a072c77c7fd8bf5839180cec29_MD5.jpg)

我们将忽略数字并仅查看颜色以指示单元格的值。现在让我们将“king”与其它单词进行比较（注意，世间有相似之人，也有相似之词）：

![](assets/images/transformer/45add25ad0472ac9fddd46261ffcb2b7_MD5.jpg)

你会发现“Man”这个词和“Woman”相比，比与“King”相比更相似，而这些向量图示很好的展现了这些单词的含义与关联

这是另一个示例列表：

![](assets/images/transformer/3b4ae9d3e22c831b1a5a07855e81bfdd_MD5.jpg)
...
![](assets/images/transformer/e8e501ad4397f9e68814d2b156d7f703_MD5.jpg)
...
![](assets/images/transformer/e8d2029b3f55cee5adf8946a3e665e7c_MD5.png)
...
![](assets/images/transformer/d66b05cf01ce1c27a2e87793b78fe2ae_MD5.jpg)

当这个窗口沿着文本滑动时，我们就能(真实地)**生成一套用于模型训练的数据集**。

不用多久，我们就能得到一个较大的数据集，从数据集中我们能看到在不同的单词组后面会出现的单词：

![](assets/images/transformer/f87852261bb2c5ef347926e55428615b_MD5.jpg)

在实际应用中，模型往往在我们滑动窗口时就被训练的，而怎么训练模型呢？

举个例子，请你根据下面这句话前面的信息进行填空：

![](assets/images/transformer/5ac1306e01b1b032ce640aac979e0927_MD5.png)

在空白前面，我提供的背景是五个单词(如果事先提及到‘bus’)，可以肯定，大多数人都会把bus填入空白中。但是如果我再给你一条信息——比如空白后的一个单词，那答案会有变吗？

![](assets/images/transformer/521be58bc3ebe619b1ef35ab1ea550a7_MD5.png)
...
![](assets/images/transformer/3974360c263c0d2fe20f0d02d834b69d_MD5.png)

如果这么做，我们实际上构建并训练的模型就如下所示：

![](assets/images/transformer/8ab23e56b7f40d7eea51e82049b02c5f_MD5.png)
...
> ![](assets/images/transformer/3a05fa9e0ba85561984fd4b927aacd78_MD5.png)

还有另一种架构，刚好反过来，根据当前词推测当前单词可能的前后单词，这种架构就是所谓的Skipgram架构

![](assets/images/transformer/b249dd91bcd1580bc8b360fd56ab816c_MD5.png)
...
2. 第二个维度，每个词其嵌入的长度即embedding\_size，比如300是一个常见值（当然，我们在前文也看过50的例子，比如上文1.1节中最后关于单词“king”的词嵌入长度）
   ...
   ![](assets/images/transformer/46798ec25cc22fbfda16551feaa0c64c_MD5.png)
...
1. 第一步，先用随机值初始化这些矩阵。在每个训练步骤中，我们采取一个相邻的例子及其相关的非相邻例子  
   ...
   ![](assets/images/transformer/c16874e0304388bf5ea54cc204b4ef85_MD5.png)
   ...
   ![](assets/images/transformer/4da285ff84827d16dc064548120b2fb2_MD5.jpg)
...
2. 第二步，计算输入嵌入与每个上下文嵌入的点积
   ...
   ![](assets/images/transformer/f1bdc252f47440bd467484cf92ecfa31_MD5.png)

   为了将这些分数转化为看起来像概率的东西——比如正值且处于0到1之间，可以通过sigmoid这一逻辑函数转换下。

   ![](assets/images/transformer/0813eac0ee0498fc409d021262d0093d_MD5.png)
...
3. 第三步，既然未经训练的模型已做出预测，而且我们拥有真实目标标签来作对比，接下来便可以计算模型预测中的误差了，即让目标标签值减去sigmoid分数，得到所谓的损失函数
   ...
   ![](assets/images/transformer/fd6a975a1cf279e5f4850a2e66918192_MD5.png)

   **error = target - sigmoid\_scores**

4. 第四步，我们可以利用这个错误分数来调整not、thou、aaron和taco的嵌入，使下一次做出这一计算时，结果会更接近目标分数

   ![](assets/images/transformer/b63a62ad53b699a20454fa02bfcfa789_MD5.jpg)

   训练步骤到此结束，我们从中得到了这一步所使用词语更好一些的嵌入（not，thou，aaron和taco）

5. 第五步，针对下一个相邻样本及其相关的非相邻样本再次执行相同的过程

   ![](assets/images/transformer/7ccf738d91802c1ad8a7fbf5530dcc39_MD5.png)
...
![](assets/images/transformer/6f499fde032339f85b299d8763c555aa_MD5.gif)
...
![](assets/images/transformer/24cbf60fdb182ca8df5e22063a31479e_MD5.png)
...
>
> ![](assets/images/transformer/9f4d835b03cf34d9f303df8f2009387c_MD5.jpg)
>
>  输入是x，经过变换$`Wx+b`$和激活函数f，得到输出y。相信大家对这个已经非常熟悉了。
>
> 在实际应用中，我们还会遇到很多序列形的数据：
>
> ![](assets/images/transformer/7eb52d3adc295d26bba1a624c68ecbce_MD5.png)
>
>  如：
>
...
> 先从$`h_{1}`$计算开始看：
>
> ![](assets/images/transformer/68ea4a796f4f321bba91094eb9596701_MD5.png)
>
> RNN可以被看做是上述同一神经网络的多次复制，每个神经网络模块会把消息传递给下一个：
>
> ![](assets/images/transformer/8ef00669ec2c7b35c995c09a29a3ded6_MD5.png)
>
> 当然，更多通常是其变种LSTM或者GRU
>
> ![](assets/images/transformer/42a81fda78cf5263c94c4b53d8c30c25_MD5.png)
...
![](assets/images/transformer/271317ff7a4defa3ea93b4794e11e409_MD5.png)
...
2. 目标句子中的每个单词都应该学会其对应的源语句子中单词的注意力分配概率信息。这意味着在生成每个单词$`y_{i}`$的时候，原先都是相同的中间语义表示C会被替换成根据当前生成单词而不断变化的$`C_{i}`$（注：这里就是Attention模型的关键，即由固定的中间语义表示C换成了根据当前输出单词来调整成加入注意力模型的变化的$`C_{i}`$）。
   ...
   ![](assets/images/transformer/014f4a7b562d69685de1bad63acdae2e_MD5.png)

3. 生成目标句子单词的过程成了下面的形式：

   ![](assets/images/transformer/b2c172d8f1f52ef1ee490a99f05c9e44_MD5.png)

   而每个$`C_{i}`$可能对应着不同的源语句子单词的注意力分配概率分布，比如对于上面的英汉翻译来说，其对应的信息可能如下： 

   ![](assets/images/transformer/28bdbd7107b9a884eb7b0623872ef9eb_MD5.png)
   ...
   ![](assets/images/transformer/fa1e392270b62d2ca973c41e850e71de_MD5.png)
...
4. 假设$`C_{i}`$下标i就是上面例子所说的“ 汤姆” ，那么$`L_{x}`$就是3，h1=f(“Tom”)、h2=f(“Chase”)、h3=f(“Jerry”)分别是输入句子每个单词的语义编码，对应的注意力模型权值则分别是0.6,0.2,0.2，所以g函数本质上就是个加权求和函数。如果形象表示的话，翻译中文单词“汤姆”的时候，数学公式对应的中间语义表示$`C_{i}`$的形成过程类似下图。
   ...
   ![](assets/images/transformer/e2b74978dcefbe4e6c14f56d9ee43f95_MD5.png)
...
>
> ![](assets/images/transformer/4ee1ec24925721d45cbf191f26b3b14f_MD5.png)
>
> 那么用下图便可以较为便捷地说明注意力分配概率分布值的通用计算过程。
>
> ![](assets/images/transformer/edf65186f1740032d70a9665a14dc5f9_MD5.png)
>
> 对于采用RNN的Decoder来说
>
...
- 至于注意力分配概率的分布数值，有多种不同的计算方法，具体参见此文：[Attention? Attention!](https://lilianweng.github.io/posts/2018-06-24-attention/ "Attention? Attention!")，或者参看参考文献14（33min至37min，以及50min都有讲）
  ...
  ![](assets/images/transformer/e9e771e25442e2dfc1470b2cc422ed2e_MD5.png)
...
![](assets/images/transformer/bbe11a2bae24b43f3d943b5e9a96fd43_MD5.png)
...
![](assets/images/transformer/687fa78458782791c69f537dd780d0c3_MD5.jpg)

整个过程具体如下图所示：

![](assets/images/transformer/d6cd76d803bb8ad54e57aa67d01ed808_MD5.png)
...
2. 第二步：将权值进行归一化（将原始计算分值整理成所有元素权重之和为1的概率分布，或者说通过SoftMax的内在机制更加突出重要元素的权重），得到直接可用的权重  
   ...
   ![](assets/images/transformer/fb7b40a52ecf867d81671a218af0edb5_MD5.jpg)

3. 第三步：将权重和 value 进行加权求和  

   ![](assets/images/transformer/cdce655d5f7216fc787b1793ce51f146_MD5.jpg)

值得一提的是，Attention 并不一定要在 Encoder-Decoder 框架下使用的，他是可以脱离 Encoder-Decoder 框架的。

![](assets/images/transformer/ec83795b5d76dba74f63b03e5909b402_MD5.png)
...
![](assets/images/transformer/7c4e16d736617d56d7f0dcb7e2e559db_MD5.png)

当我们拆开这个黑箱后，我们可以看到它是由编码组件、解码组件和它们之间的连接组成。

![](assets/images/transformer/15eac347175fa9bb817d973431a52b44_MD5.png)
...
![](assets/images/transformer/3f30702e3b3e8a45025c0a8f41ed21cb_MD5.png)

进一步拆开这个编码器会发现，所有的编码器在结构上都是相同的，但是并不共享参数，且每个编码器都可以分解成两个子层：

![](assets/images/transformer/f3eb7c90b409dbbe7f6a4ebbbbea417c_MD5.jpg)

从而，当我们把编码器和解码器联合起来看待的话，则整个流程就是（如下图从左至右所示）：

![](assets/images/transformer/60312199b92e2955a3ffb2f8a1217aec_MD5.jpg)
...
![](assets/images/transformer/54d783622495456a4ebe4f78e0954228_MD5.png)
...
![](assets/images/transformer/afee7380b23f96110e9593354f6c1c99_MD5.jpg)

此时，我们可以看出Transformer的一个核心特性

![](assets/images/transformer/03477feb6607035c09e0647ed51c0717_MD5.jpg)
...
![](assets/images/transformer/d9c6037ac0cea0dad506b75496c6edfa_MD5.png)
...
![](assets/images/transformer/e2f3ed201af4091433fa0c2e552c1888_MD5.jpg)
...
![](assets/images/transformer/fadc2266e03a0d37020d56c897df0a34_MD5.jpg)
...
![](assets/images/transformer/40bfd7d3ee8fab25f70ad4e47e6b7a67_MD5.jpg)
...
![](assets/images/transformer/3361f4f6b1c1740ddeae3e3ba81eeea5_MD5.png)

再次提醒：词嵌入向量 (512，或图中的4个格子)，和q/k/v向量(64，或图中的3个格子)的大小差异

最后，由于我们处理的是矩阵，我们可以将步骤2到步骤6合并为一个公式来计算自注意力层的输出，下图是自注意力的矩阵运算形式：

![](assets/images/transformer/28f360e43e6a8ce32d68f8a7cd7a45b9_MD5.png)
...
![](assets/images/transformer/4db158cf962f1262b5d9b53e63e7be81_MD5.jpg)
...
![](assets/images/transformer/db937132cff0d9444788b4a43d0f0fb9_MD5.png)
...
![](assets/images/transformer/17c9ff5d3d01c44fa3fed1bdf9e93ebc_MD5.jpg)

这几乎就是多头自注意力的全部，接下来把所有矩阵集中展示下，如下图所示

![](assets/images/transformer/7fc3a202f4d625dd2c204d268f444573_MD5.jpg)

现在我们已经看过什么是多头注意力了，让我们回顾一下之前的一个例子：“The animal didn’t cross the street because it was too tired”，再看一下编码“it”的时候每个头的关注点都在哪里：

![](assets/images/transformer/e8de3f53932a45a3e07add8602220d08_MD5.png)
...
![](assets/images/transformer/c575fe306df6fe8dd9475d84c2b1785c_MD5.png)
...
>
>    ![](assets/images/transformer/85065d3ce99f9487e4b742c4cd25888f_MD5.png)
>
> 2. 且每次往右移动一格
>
>    ![](assets/images/transformer/51b6f11a315c19a269a7b103b377e368_MD5.png)
>
...
>
>    ![](assets/images/transformer/081711acdffbd86d5dd901ab11f12e45_MD5.png)
>
> 4. 怎么办呢？好在不管怎么着，特征A1和特征A9之间的距离相较于“小明”和“他”的距离变近了。所以我们可以继续在特征A上再堆叠一层卷积B 然后计算，然后再堆叠卷积C 再计算，直到卷积操作比如E能直接获取到“小明”和“他”的信息
>
>    ![](assets/images/transformer/e6c505de964ca005b5c72021a7082ced_MD5.png)
...
![](assets/images/transformer/71b156192cbbc0427bab79e11815a346_MD5.png)
...
![](assets/images/transformer/d9bae5ae137038398e8ac3118191d6e8_MD5.png)
...
![](assets/images/transformer/695f99dca949276ba8466607f46742a0_MD5.jpg)

这个过程中的向量“求和层归一化”的示意图如下所示：

![](assets/images/transformer/c56f16d937169ee283b7dbdc7785700a_MD5.jpg)

当然在解码器子层中也是这样的，从而当有着两个编码器和两个解码器的Transformer就是下图这样的：

![](assets/images/transformer/1ca2f383a4a6d73fe29aaff772304f62_MD5.png)

 现在我们已经介绍了编码器的大部分概念，由于encoder的decoder组件差不多，所以也就基本知道了解码器的组件是如何工作的。最后，我们直接看下二者是如何协同工作的：

![在这里插入图片描述](assets/images/transformer/ad73f95847f33a27bdef611c1a07f484_MD5.gif)
...
![](assets/images/transformer/9ff0f2f885122ef541024b679cdf7c5c_MD5.jpg)
...
![](assets/images/transformer/b675304324148d1ddf10e68dc33b8153_MD5.png)
...
![](assets/images/transformer/95a2ef7d1fadd567d04b368ef22cefd7_MD5.png)
...
![](assets/images/transformer/ca3fe94491cef6b86588816106c17a85_MD5.png)
...
![](assets/images/transformer/9d3070c4862b419e382b8b89ebeb0f04_MD5.png)
...
![](assets/images/transformer/77c0c9a43d2a8f2d5870056e8c5d8377_MD5.png)
...
>
> ![](assets/images/transformer/03835827fb44e543e70cde4c24f4958b_MD5.png)
>
> ___
>
...
![](assets/images/transformer/5836e4fade679f7fdcdc37360768fd5c_MD5.png)
...
![](assets/images/transformer/5f1954870d53077ba0ea0483301816a9_MD5.png)
...
-   第二点是预训练的时候采用双向语言模型  
    ![](assets/images/transformer/b54a295fecf8687cc24421d893f4dab6_MD5.png)
...
2. 和GPT的最主要不同在于在预训练阶段采用了类似ELMO的双向语言模型，当然另外一点是语言模型的数据规模要比GPT大
   ...
   ![](assets/images/transformer/690c0782107b43044ce8584da9dc943f_MD5.png)
...
![](assets/images/transformer/d6431465c1e7bec6d952041358c9edf8_MD5.png)
...
![](assets/images/transformer/4059740ccdd04f6648aa5c9c5ac89ef2_MD5.jpg)

把单词对应的三个embedding叠加（没错，直接相加），就形成了BERT的输入。

![](assets/images/transformer/deaa8c2b8634b11e28524700ce181ce7_MD5.png)

至于BERT在预训练的输出部分如何组织，可以参考上图的注释

![](assets/images/transformer/a550ff894ef7c4dd5fb72473e3fca0d5_MD5.jpg)
...
![](assets/images/transformer/3919032d75e33cf2d945e264d1cbef6d_MD5.png)
...
![](assets/images/transformer/e5e7f107f3f0281d2892fde09ce474ef_MD5.jpg)
...
![](assets/images/transformer/d9f1735f332c29db399e072817da3144_MD5.jpg)
...
> ![](assets/images/transformer/34b76d54bf8f141493febb05dcbf0d20_MD5.jpg)
